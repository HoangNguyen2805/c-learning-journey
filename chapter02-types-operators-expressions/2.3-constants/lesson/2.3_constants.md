...
- enum = A user-defined data type that consists of a set of named integer contants.
Benefit: Replace number with readable names.

How it wor ?
```c
enum [tag name]{
    [CONSTANT VARIABLE] -> listed in order which represent by number from 0,1,2,3 ( left to right )
};

int main(){

    enum [tag name] [new variable] = [CONSTANT VARIABLE];

    printf("%d", [new variable]);

    return 0;
}

// [tag name] = data type
// it will print out number that worth with the [CONSTANT VARIABLE]'s list.
...
BUT we can also assign numer to it to:
Example:
```c
#include <stdio.h>
...
3
```
LOOK up Youtube enum + type def work too
while loop 
Function
switch too
if else 
